  ...
  knotXNodes.map(item => {
    item.addEventListener(
      ...
      function(ev) {
        ...
        document.getElementById("knotx-snippet").innerHTML = "<xmp>" +
            debugData[ev.currentTarget.dataset.knotxId].snippet + "</xmp>";
        document.getElementById("knotx-data").innerHTML = "<xmp>" +
            JSON.stringify(debugData[ev.currentTarget.dataset.knotxId].data, null, 2) +
            "</xmp>";
      },
      true
  );
...
function createTemplate() {
  ...
  newDiv.innerHTML =
      "<h1>Welcome to Knot.X debug mode</h1><div id='knotx-snippet'></div><div id='knotx-data'></div>";
